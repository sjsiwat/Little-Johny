/**
 * LINE Login OAuth callback — Cloudflare Pages Function
 *
...
export async function onRequestGet({ request, env }) {
  ...
  try {
    ...
    if (Array.isArray(lineUsers) && lineUsers.length > 0) {
      ...
      // Refresh display name and picture from LINE
      await fetch(
    ...
    // 4. Fetch the user's email from Supabase auth
    ...
    // 5. Generate a magic link so the browser gets a real Supabase session
